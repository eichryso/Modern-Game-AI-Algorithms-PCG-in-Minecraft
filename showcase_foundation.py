...
def main():
    """
    Main function to showcase the foundation of the house by building only the foundation and walls for each floor, without adding furniture or stairs. The function scans the building area for suitable locations to place the house,
    and for each valid location, it builds the foundation and walls for each floor of the house, as well as the roof, while respecting the constraints of the building area. 
    The garden room is also built to showcase the exterior structure. We do not use the scan_terrain function as we test on a flat world.
    """
    ...
    for grid_x in range(0, build_length - house_length, house_length + 1):
        for grid_z in range(0, build_width - house_width, house_width + 1):
            ...
            for floor in range(len(floor_dims)):
                ...
                build_floor(ED, dims, floor_y, house_height, num_floors, floor, stair_x, stair_z, stair_positions, wall_material=wall_material, showcase_foundation=True, floor_material=floor_material)
